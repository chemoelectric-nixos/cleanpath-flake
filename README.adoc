...
----
cleanpath = {
  url = "github:chemoelectric-nixos/cleanpath-flake/v1.0.6?dir=build%2Fx86_64-linux%2Frelease-26.05 ";
};
----
...
gcc compilers of NixOS. We compile the program statically with a D
compiler.footnote:[At NixOS, they cannot even keep GNU D working at
all, so it is not that.]
